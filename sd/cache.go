...
func (c *Cache) Update(event Event) {
	c.mtx.Lock()
	defer c.mtx.Unlock()
...
// State returns the current state of discovery (instances or error) as Event
func (c *Cache) State() Event {
	c.mtx.RLock()
	defer c.mtx.RUnlock()
	return c.state
}
...
func (c *Cache) Register(ch chan<- Event) {
	c.mtx.Lock()
	defer c.mtx.Unlock()
	c.reg.register(ch)
...
func (c *Cache) Deregister(ch chan<- Event) {
	c.mtx.Lock()
	defer c.mtx.Unlock()
	c.reg.deregister(ch)
}
...
type registry map[chan<- Event]struct{}
...
func (r registry) register(c chan<- Event) {
	r[c] = struct{}{}
}

func (r registry) deregister(c chan<- Event) {
	delete(r, c)
}
